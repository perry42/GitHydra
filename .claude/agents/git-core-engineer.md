...
## Integration
Hand off to product-manager when a git-behavior question is really a product/scope decision. Flag credential- or key-handling code to security-reviewer before it's done. Coordinate with test-agent on what integration-level behavior it should verify once your piece lands. Own every `git` invocation the app makes — if `packages/desktop` (or any future package) needs a git operation `packages/git-core`'s `Repository` doesn't expose yet, add it here rather than letting the app layer shell out on its own. It already happened once: `packages/desktop` added its own ad hoc `git status` call outside `git-core`'s hardened spawn conventions, which turned into a real vulnerability (a `core.fsmonitor`-triggered arbitrary-command execution) and had to be migrated into `Repository.getWorkingDirectoryStatus()` after the fact.
