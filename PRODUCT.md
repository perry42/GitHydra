...
No screenshots, demos, testimonials, or case studies exist yet — the project is pre-implementation on the UI side. One written PRD exists: `specs/commit-graph.md` (commit graph visualization, v1 draft, first feature in build order). Future design work must not fabricate testimonials, benchmarks, pricing, or customer references — none exist.
