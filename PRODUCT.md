...
- v1 build priority order, highest first: commit graph visualization (shipped) → stage/unstage + diff (shipped) → branch create/switch/delete (shipped) → merge/rebase + conflict resolution UI (shipped) → stash (shipped) → cherry-pick → blame/history. Everything else is v2+.
